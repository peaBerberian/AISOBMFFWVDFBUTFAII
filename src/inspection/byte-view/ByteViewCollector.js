 ...
 * @typedef {{
 *   captureLimit: number,
 *   captureBudgetExceeded: boolean,
 *   partialCaptureMessage: string,
 *   capturedSpans: CapturedByteSpan[],
 *   fieldsById: Map<string, IndexedFieldSpan>,
 *   fieldSpans: IndexedFieldSpan[],
 * }} ByteViewRenderData
 */

export default class ByteViewCollector {
  #captureLimit;
  ...
  /**
   * @returns {ByteViewRenderData | null}
   */
  toRenderData() {
    if (this.#capturedSpans.length === 0) {
      return null;
    }
    return {
      captureLimit: this.#captureLimit,
      captureBudgetExceeded: this.#captureBudgetExceeded,
      partialCaptureMessage: `Byte View retained up to ${fmtBytes(this.#captureLimit)} of non-mdat metadata bytes for this inspection. Some fields are outside the capture budget and cannot be shown here.`,
      capturedSpans: this.#capturedSpans,
      fieldsById: new Map(this.#fieldsById),
      fieldSpans: this.#fieldSpans.slice(),
    };
  }
  ...
  #indexBoxFields(box, boxPath) {
    ...
    const boxOrder = this.#nextBoxOrder++;
    this.#indexBoxHeaderFields(box, boxPath, boxKey, boxOrder);
   ...
   */
  #indexBoxHeaderFields(box, boxPath, boxKey, boxOrder) {
    ...
    this.#registerSyntheticField({
      ...
      description: "Declared box size field from the box header.",
      summary:
    ...
    this.#registerSyntheticField({
      ...
      description: "Four-character box type from the box header.",
      summary: box.type,
    });
  }
...
function getFieldDescription(field) {
  return "description" in field && typeof field.description === "string"
    ? field.description
    : "";
}
