...
import {
  ...
  getFieldAnnotation,
  getPsshSystemIdInfo,
} from "./box_decorations";
import {
  declareLazyBox,
  openBoxBody,
  shouldAutoOpenBox,
} from "./box_opening.js";
import { renderPsshPreviewField } from "./pssh";
import { hidePropertyTooltip, showPropertyTooltip } from "./tooltip";
 ...
 */
export default function renderBoxTreeNode(box, options = {}) {
  ...
  if (hasChildren) {
    for (const child of box.children ?? []) {
      childContainer.appendChild(
        renderBoxTreeNode(child, { shallow: false }).element,
      );
    }
  }
...
/**
 * @param {import("isobmff-inspector").ParsedField | import("./pssh").PsshPreviewField | null} f
