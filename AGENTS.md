...
- `src/inspection/InspectionRun.js`: lifecycle state and typed actions
  handling for one inspection; owns parser collector state and final result
  assembly.
- `src/inspection/parseInspectionStream.js`: streams parser events into
  `InspectionRun` without importing UI modules.
...
- `src/sources/hls_resource.js`: resolves HLS source choices and reports
  source-resolution actions.
- `src/sources/remote_segment.js`: fetches direct or chosen remote segments and
  starts parsing.
...
- `src/post-process/projections.js`: completed semantic projections consumed by
  result tabs.
...
4. `src/inspection/parseInspectionStream.js` streams parser events into
   `InspectionRun`; parser events and payload chunks use specialized
...
`InspectionRun` events handled by `InspectionCoordinator`. Derived tabs are
rendered after parsing finishes, using completed projections from
